...
%
% function FilterAspec: Updates selected aspect ratio range and filters
% ellipsoids depending on their aspect ratio.
...
if str2double(get(Text41,'string'))> str2double(get(Text43,'string')) && str2double(get(Text43,'string')) ~= 0 % Security if entered min aspec > max aspec.
    set(Text8, 'string', 'Error: min Aspect ratio is larger than max. Aspect ratio', 'Foregroundcolor','r') % Update text box
    V1=[]; % Clear variables
    V2=[];
    V3=[];
    Vtot=[];
...
if str2double(get(Text41,'string')) == 0 % If entered aspect ratio is 0: set back minimal blob aspect ratio
    params.aspecran(1)=min(cell2mat(data.raw(:,8))./cell2mat(data.raw(:,10)))-0.00001;
elseif str2double(get(Text41,'string')) > min(cell2mat(data.raw(:,8))./cell2mat(data.raw(:,10))) % Otherwise, update params.aspecran with the entered value
    params.aspecran(1)=str2double(get(Text41,'string'))-0.00001;
end

if str2double(get(Text43,'string')) == 0 % If entered volume is 0: set back maximal blob volume
    params.aspecran(2)=max(cell2mat(data.raw(:,8))./cell2mat(data.raw(:,10)))+0.00001;
elseif str2double(get(Text43,'string')) < max(cell2mat(data.raw(:,8))./cell2mat(data.raw(:,10))) % Otherwise, update params.aspecran with the entered value
    params.aspecran(2)=str2double(get(Text43,'string'))+0.00001;
end

data.filtered={};
j=1;
for i=1:size(data.raw,1)
    if cell2mat(data.raw(i,8))/cell2mat(data.raw(i,10))>=params.aspecran(1) && cell2mat(data.raw(i,8))/cell2mat(data.raw(i,10))<=params.aspecran(2) % Filter blobs depending on the volume and record in data.filtered
        if cell2mat(data.raw(i,4))>=params.volran(1) && cell2mat(data.raw(i,4))<=params.volran(2) % Keep the volume filter that is already on.
...
set(Text4,'string',[num2str(size(data.filtered,1)) ' ellipsoids after vol. & asp. ratio filtering']) % Refresh GUI
...
set(Text43,'string',num2str(round(params.aspecran(2),4))) % Refresh GUI %%STOP HERE

...
%
% function ShapeParams: Calculate various shape parameters on blobs and mean orientation tensor
...
params.Kmean=log(V1.eigval(3)/V1.eigval(2))/log(V1.eigval(2)/V1.eigval(1));
...
V1.PGR=zeros(3,1);
V1.PGR(1)=V1.eigval(3)-V1.eigval(2);
V1.PGR(2)=2*(V1.eigval(2)-V1.eigval(1));
V1.PGR(3)=3*V1.eigval(1);

V2.PGR=zeros(3,1);
V2.PGR(1)=V2.eigval(3)-V2.eigval(2);
V2.PGR(2)=2*(V2.eigval(2)-V2.eigval(1));
V2.PGR(3)=3*V2.eigval(1);

V3.PGR=zeros(3,1);
V3.PGR(1)=V3.eigval(3)-V3.eigval(2);
V3.PGR(2)=2*(V3.eigval(2)-V3.eigval(1));
V3.PGR(3)=3*V3.eigval(1);
...
if strcmp(params.ormeth,'tensor') % Calculate the T and P' parameters for the mean orientation tensor if tensor analysis is selected
    params.Tmean=(log(Vtot.eigval(2)/Vtot.eigval(3))-log(Vtot.eigval(1)/Vtot.eigval(2)))/(log(Vtot.eigval(2)/Vtot.eigval(3))+log(Vtot.eigval(1)/Vtot.eigval(2)));
    mean=(Vtot.eigval(3)+Vtot.eigval(2)+Vtot.eigval(1))/3;
    params.Pjmean=exp(sqrt(2*((log(Vtot.eigval(1)/mean))^2+(log(Vtot.eigval(2)/mean))^2+(log(Vtot.eigval(3)/mean))^2)));

...
V1.eigval=abs(sort(eig(V1.ortens)));
...
V2.eigval=abs(sort(eig(V2.ortens)));
...
V3.eigval=abs(sort(eig(V3.ortens)));

% Compiling mean fabric tensor
Vtot.fabtens=zeros(3,3); % Mean fabric tensor
Vtot.nofabtens=zeros(3,3); % Normalized mean fabric tensor
...
%% Axis (vector) approach following Fisher (1963). Estimate mean directions independently. Resulting mean directions will likely NOT be orthogonal
...
%% Tensor approach. All directions distribution are dependent. Follows the approach of Scheidegger (1965) and many others afterwards.
...
if get(chb10,'Value') == 3
    params.ormeth='tensor'; % Update mean orientation method
    ...
    Vtot.eigval=[eigval(3,3) eigval(2,2) eigval(1,1)];

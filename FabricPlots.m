...
if params.daPj~=0 % Plot the value of the distribution anisotropy tensor
    plot(params.daK(1),params.daK(2),'ok','MarkerFaceColor',[0.7 0.7 0.7])
end


if params.Pjmean~=0 % Plot the value of the mean orientation tensor
    %     plot(log(V1.oteigval(2)/V1.oteigval(1)),log(V1.oteigval(3)/V1.oteigval(2)),'ok','MarkerFaceColor',[0 0 0])
    plot(log(Vtot.fteigval(2)/Vtot.fteigval(3)),log(Vtot.fteigval(1)/Vtot.fteigval(2)),'ok','MarkerFaceColor',[0 0 0])
else
...
if params.Pjmean~=0 % If Pj was calculated
    ...
    set(Text33, 'String',['FT.eigval = ' num2str(round(Vtot.fteigval(1),4)) ' / ' num2str(round(Vtot.fteigval(2),4)) ' / ' num2str(round(Vtot.fteigval(3),4))]) % Refresh GUI
    ...
    set(Text33, 'string' , 'FT.eigval = ... / ... / ...') % Refresh GUI
end
if params.daPj~=0 % If distribution anisotropy tensor was calculated
    set(Text31,'String',['DA.T = ' num2str(round(params.daT,2))]) % Refresh GUI

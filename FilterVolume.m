...
%
% function FilterVolume: Updates selected volume range and filters
...
for i=1:size(data.raw,1)
    if cell2mat(data.raw(i,4))>=params.volran(1) && cell2mat(data.raw(i,4))<=params.volran(2) % Filter blobs depending on the volume and record in data.filtered
        if cell2mat(data.raw(i,8))/cell2mat(data.raw(i,10))>=params.aspecran(1) && cell2mat(data.raw(i,8))/cell2mat(data.raw(i,10))<=params.aspecran(2) % Keep the aspect ratio filter that is already on.
